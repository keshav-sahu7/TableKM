    ...
    : K2QtTableModel(table, path, TableType::BASIC_VIEW, parent)
{
    ...
    connect(notifier, &ViewChangeNotifier::kDataChanged, [this](auto r, auto c){
        emit dataChanged(index(r,c),index(r,c));
    });
    connect(notifier, &ViewChangeNotifier::kRowInserted, [this](auto r){
        beginInsertRows(QModelIndex(),r,r);
        endInsertRows();
    });
    connect(notifier, &ViewChangeNotifier::kRowDropped, [this](auto r){
        beginRemoveRows(QModelIndex(),r,r);
        endRemoveRows();
    });
    connect(notifier, &ViewChangeNotifier::kCompleteUpdateNeeded, [this]{
        beginResetModel();
        endResetModel();
    });
    connect(notifier, &ViewChangeNotifier::kColumnTransformed, [this](auto c){

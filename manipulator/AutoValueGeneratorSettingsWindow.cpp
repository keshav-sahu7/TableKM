      ...
      m_evaluator(evaluator)
{
    ...
    m_ui->btn_attacher->setText(as_dialog ? tr("Attach To Tab") : tr("Detach"));
    ...
    connect(m_ui->btn_attacher, &QPushButton::clicked, this, &AutoValueGeneratorSettingsWindow::attacherButtonClicked);
}
...
void AutoValueGeneratorSettingsWindow::attacherButtonClicked()
{
    auto flags = windowFlags();
    if(flags.testFlag(Qt::Dialog))
    {
        auto prnt = dynamic_cast<QWidget*>(parent());
        QRect geom = QRect(QPoint(0,0),prnt->size());
        flags.setFlag(Qt::Dialog, false);
        flags.setFlag(Qt::Widget, true);
        setGeometry(geom);
        m_ui->btn_attacher->setText(tr("Detach"));
    }
    else{
        flags.setFlag(Qt::Widget, false);
        flags.setFlag(Qt::Dialog, true);
        m_ui->btn_attacher->setText(tr("Attach"));
    }
}
...
void AutoValueGeneratorSettingsWindow::on_btn_apply_clicked()
{

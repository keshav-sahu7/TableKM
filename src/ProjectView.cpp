...
ProjectView::ProjectView(MainWindow *main_window, QWidget *parent) : QTreeView(parent)
{
    ...
    connect(ac_delete, &QAction::triggered, this, [this, ac_delete, main_window](){
        K2QtTableModel *model = ac_delete->data().value<K2QtTableModel*>();
        K_RETURN_IF_NOT(model);
        auto item = model->getProjectedItem();
    ...
    connect(ac_close_table, &QAction::triggered, this, [this, ac_close_table, main_window](){
        K2QtTableModel *model = ac_close_table->data().value<K2QtTableModel*>();
        K_RETURN_IF_NOT(model);
        auto item = model->getProjectedItem();
        this->removeItem(item);
        model->deleteLater();
